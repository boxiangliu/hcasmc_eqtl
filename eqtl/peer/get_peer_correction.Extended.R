...
if (length(args) < 4) {
  cat("Usage: R -f get_peer_correction.Extended.R --slave --vanilla --args TraitsFileName MaxFactorsN MaxIterations BoundTol VarTol e_pa e_pb a_pa a_pb OutDir\n", file=stderr())
  quit(status=2)
}
...
GetPeerModel <- function(m1, MaxFactorsN, MaxIterations, a_pa, a_pb, e_pa, e_pb){
	...
	PEER_setPriorAlpha(model,a_pa,a_pb); # weight precision
	if (!is.null(res)) RiseError("not correct")
	cat("PEER_setPriorEps\n")
	PEER_setPriorEps(model,e_pa, e_pb); # noise precision
	if (!is.null(res)) RiseError("not correct")
...
cat("get_peer_correction.Extended.R DONE\n")
