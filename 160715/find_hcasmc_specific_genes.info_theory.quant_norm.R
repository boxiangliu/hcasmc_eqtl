...
# Quantile normalization (takes time): 
rpkmn=as.data.table(normalize.quantiles(as.matrix(rpkm)))
colnames(rpkmn)=coldata$sample
...
# Save ESI: 
fwrite(data.table(gene_id=names(esi),esi=esi),'../processed_data/160715/esi.quant_norm.hcasmc.txt',sep='\t')
...
fwrite(esi_df,'../processed_data/160715/esi.quant_norm.all_tissues.txt',sep='\t')


# Make qqplot:
p=make_qqplot(esi_ls)
save_plot('../figures/160715/esi_distribution_qqplot.quant_norm.pdf',p)
...
save_plot('../figures/160715/rpkm_vs_rank.quant_norm.pdf',p)