...
subset=to_plot
subset_cast=dcast(subset[!str_detect(data,'ATAC'),],chr+pos+rsid~data,value.var='logp',fun.aggregate=max)

p1=ggplot(subset_cast,aes(Nikpay,RASQUAL))+geom_point()
p2=ggplot(subset_cast,aes(Howson,RASQUAL))+geom_point()


pdf(sprintf('%s/snhg18.pdf',fig_dir))
p1;p2
dev.off()
...
eqtl_fn='../processed_data/rasqual/output_pval/chr11/ENSG00000172803.13_SNX32.pval.txt'
eqtl=fread(eqtl_fn)
eqtl[,logp:=-log10(pval)]
...
pdf(sprintf('%s/snx32.pdf',fig_dir))
p1;p2
dev.off()
...
eqtl_fn='../processed_data/rasqual/output_pval/chr10/ENSG00000166275.11_C10orf32.pval.txt'
eqtl=fread(eqtl_fn)
eqtl[,logp:=-log10(pval)]
...
pdf(sprintf('%s/C10orf32.pdf',fig_dir))
p1;p2
dev.off()
...
eqtl_fn='../processed_data/rasqual/output_pval/chr17/ENSG00000167720.8_SRR.pval.txt'
eqtl=fread(eqtl_fn)
eqtl[,logp:=-log10(pval)]
...
eqtl_fn='../processed_data/rasqual/output_pval/chr2/ENSG00000138036.14_DYNC2LI1.pval.txt'
eqtl=fread(eqtl_fn)
eqtl[,logp:=-log10(pval)]
...
pdf(sprintf('%s/dync2li1.pdf',fig_dir))
p1;p2
dev.off()