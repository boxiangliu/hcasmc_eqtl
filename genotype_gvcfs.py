...
def genotypeGVCFs(sample_list, out_vcf = 'raw_variants.vcf', dry_run = False):
	cmd = "java -Xmx32g -jar %s -nt 8 -T GenotypeGVCFs -R %s --dbsnp %s/dbsnp_138.hg19.vcf"%(GATK,HG19,GATK_BUNDLE)
...
def calibrateSNPs(vcf, recal_vcf, tranche = 99, dry_run = False):
	cmd = "java -Xmx32g -jar %s -nt 8 -T VariantRecalibrator -R %s -input %s -resource:hapmap,known=false,training=true,truth=true,prior=15.0 %s/hapmap_3.3.hg19.sites.vcf -resource:omni,known=false,training=true,truth=true,prior=12.0 %s/1000G_omni2.5.hg19.sites.vcf -resource:1000G,known=false,training=true,truth=false,prior=10.0 %s/1000G_phase1.snps.high_confidence.hg19.sites.vcf -resource:dbsnp,known=true,training=false,truth=false,prior=2.0 %s/dbsnp_138.hg19.vcf -an DP -an QD -an FS -an SOR -an MQ -an MQRankSum -an ReadPosRankSum -an InbreedingCoeff -mode SNP -tranche 100.0 -tranche 99.9 -tranche 99.0 -tranche 90.0 -recalFile recalibrate_SNP.recal -tranchesFile recalibrate_SNP.tranches -rscriptFile recalibrate_SNP_plots.R"%(GATK,HG19,vcf,GATK_BUNDLE,GATK_BUNDLE,GATK_BUNDLE,GATK_BUNDLE)
	call(cmd,dry_run)
