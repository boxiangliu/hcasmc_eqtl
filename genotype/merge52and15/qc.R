...
fig_dir='../figures/genotype/merge52and15/'
if (!dir.exists(fig_dir)) {dir.create(fig_dir,recursive=TRUE)}

res=foreach(i = c(seq(22),'X'))%dopar% {
	cmd=sprintf('bcftools view -H %s/67total/chr%s.15.vcf.gz | wc -l',wd,i)
	nline_array=as.integer(system(cmd,intern=TRUE))

	cmd=sprintf('bcftools view -H %s/67total/chr%s.52.vcf.gz | wc -l',wd,i)
	nline_wgs=as.integer(system(cmd,intern=TRUE))

	cmd=sprintf('bcftools view -H %s/67total/chr%s.vcf.comm.gz | wc -l',wd,i)
...
res2=foreach(i = c(seq(22),'X'))%dopar% {
	cmd=sprintf('bcftools view -H --min-af 0.05 %s/67total/chr%s.vcf.comm.gz | wc -l',wd,i)
	nline_intersected_maf=as.integer(system(cmd,intern=TRUE))

	cmd=sprintf('bcftools view -H --min-af 0.05 %s/67total/chr%s.vcf.gz | wc -l',wd,i)
