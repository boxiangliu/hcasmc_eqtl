...
OUT_DIR='../processed_data/hcasmc_specific_open_chromatin/encode_plus_hcasmc_filt_all/'
...
filter_metadata_without_taking_max=function(x){
	...
	z=y[,.(`File accession`,`Biosample term name`,`Biosample type`,peak_type)]
...
# Filter metadata:
annotation=filter_metadata_without_taking_max(annotation)

# Add HCASMC: 
fn=list.files(IN_DIR,pattern='naive_overlap.narrowPeak.gz')
annotation=rbind(data.frame(file=fn,epigenome='HCASMC',type='primary cell',peak_type='raw'),annotation)
...
for (i in 1:nrow(annotation)){
	...
	input=sprintf('%s/%s',IN_DIR,annotation$file[i])
	output=paste0(OUT_DIR,'/',str_replace_all(annotation$epigenome[i],' ','_'),'.',str_split_fixed(annotation$file[i],'\\.',2)[,1],'.bed')
	message(input)
	message(output)
	is_raw=FALSE
	if (peak_type=='raw'){
